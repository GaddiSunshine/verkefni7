 ...
 */
function start() {
    alert('Velkomin í ágiskunarleikinn minn');
 do{
    play()
 } while (confirm('spilan annan?'));
...
function play() {
 ...
 do {
     const input = prompt('Hver er talan?');
     guessedInput = parseGuess(input);
         if(guessedInput === null){
            alert('hætt í leik');
         break;
         }
     alert(getResponse(guessedInput, random));
     correct = guessedInput === random;
     attempts++;    
} while (!correct);
...
function getResults(){ 
    ...
    return 'Spilaðir leikir: ' + played + '\nMeðaltal ágiskana: ' + avg;
}
